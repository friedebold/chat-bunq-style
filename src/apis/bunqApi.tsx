...
export const fetchUsers = async () => {
  return fetch('https://assignment.bunq.com/users')
    .then(jsonResponse => jsonResponse.json())
    .then((users: User[]) => users)
}

export const fetchLastSeen = async (conversationId: string, userId: string) => {
  return fetch(`https://assignment.bunq.com/conversation/${conversationId}/lastseen/${userId}`)
    .then(jsonResponse => jsonResponse.json())
...
export const fetchConversations = async (userId: string) => {
  ...
  return fetch(`https://assignment.bunq.com/conversation/user/${userId}`)
    .then(jsonResponse => jsonResponse.json())
...
export const createPersonalConversation = async (receiverId: string, userId: string,) => {
  ...
  return fetch('https://assignment.bunq.com/conversation/personal', {
    method: 'POST',
...
export const createGroupConversation = async (receiverIds: string, userId: string, groupName: string) => {
  ...
  return fetch('https://assignment.bunq.com/conversation/group', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      users: bodyData,
...
export const markConversationAsSeen = async (conversationId: string, userId: string) => {
  return fetch(`https://assignment.bunq.com/conversation/${conversationId}/seen/${userId}`, {
    method: 'PUT',
  })
    .catch(err => console.log(err))
}
...
export const fetchUsersAndLastSeen = async (users: ChatUser[], conversationId: string) => {
  ...
  for (let i = 0; i < users.length; i++) {
    promises.push(fetch(`https://assignment.bunq.com/user/${users[i].userid}`))
    promises.push(fetch(`https://assignment.bunq.com/conversation/${conversationId}/lastseen/${users[i].userid}`))
  }
...
export const sendMessage = async (conversationId: string, senderId: string, message: string) => {
  return fetch(`https://assignment.bunq.com/conversation/${conversationId}/message/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
export const getLast100Messages = async (conversationId: string, offset: number) => {
  return fetch(`https://assignment.bunq.com/conversation/${conversationId}/message/limited?limit=100&offset=${offset}`)
    .then(jsonResponse => jsonResponse.json())
    .then(response => response)
    .catch((err: Error) => console.log(err))
}
...
export const fetchNewMessages = async (conversationId: string, lastMsgId: string) => {
  return fetch(`https://assignment.bunq.com/conversation/${conversationId}/new/${lastMsgId}`)
    .then(jsonResponse => jsonResponse.json())
    .then(response => response)
    .catch((err: Error) => console.log(err))
}